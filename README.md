...
> [!WARNING]   
> 密码已通过本机唯一密钥加密存储（AES-128-CBC），无法在其他电脑上解密。  
> 但加密密钥基于本机机器名和用户名生成，更换电脑或重装系统后需重新输入密码。  
> **请务必在可信的环境下使用** 🔐
...
##  首次运行 exe 须知（Windows SmartScreen）

由于 exe 未采用代码签名证书，Windows SmartScreen 会在首次运行时拦截并显示"此应用可能危害您的设备"。

**这不是病毒，请按以下步骤放行**：

1. 双击 `PU-SignUpBot.exe` 后，看到蓝色弹窗，点击 **"更多信息"**
2. 点击右下角出现的 **"仍要运行"** 按钮
3. 之后即可正常使用，不会再次拦截

> [!NOTE]
> SmartScreen 拦截是因为未购买代码签名证书，属于正常现象，大多数小众工具都会遇到。
